...
run_initial_config() {
    local current_dir=$(pwd)
    log "Running initial configuration setup..."
    "$current_dir/setup_config.sh" --initialize --export
}
...
main() {
    ...
    echo -e "To check service status: ${BOLD}$(pwd)/autopub_monitor_tmux_session.sh status${NC}"
}
